            ...
            for (int j = 0; j < nums.length; j++) {
                if ((i != j) && ((nums[i] + nums[j]) == 8)) {
                    if (nums[i] > nums[j]) {
                        ...
                        combs = addToArr(combs, new int[]{nums[j], nums[i], Math.abs(j - i)});
    ...
    public static boolean isNew(int num){
        ...
        for (int i = 0; i < num_str.length(); i++) {
            ...
            for (int j = 0; j < num_str.length(); j++){
                ...
                if (i < j & i == 0){
                    if (Integer.parseInt(String.valueOf(num_str.charAt(i))) > Integer.parseInt(String.valueOf(num_str.charAt(j))) & !String.valueOf(num_str.charAt(j)).equals("0")){
                        return false;
                    }
                }
            }

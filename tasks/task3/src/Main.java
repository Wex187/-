...
public class Main {
    ...
    public static String replaceVovels(String original_string) {
        String new_string = original_string.replaceAll("[AEIOUaeiou]", "*");
        return new_string;
    }
    //2------------------------------------------------------------------------------------
    ...
    public static ArrayList<String> salesData(String[][] double_array) {
        ...
        for (int i = 0; i < double_array.length; i++) {
            if (double_array[i].length > maks_array) {
                maks_array = double_array[i].length;
            }
        }
        for (int i = 0; i < double_array.length; i++) {
            if (double_array[i].length == maks_array) {
                new_array.add(double_array[i][0]);
            }
        }
        return new_array;
    }
